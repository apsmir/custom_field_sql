      ...
      context[:issue].available_custom_fields.each do |field|
        if field.is_a?(IssueCustomField)
          if field.field_format == 'sql_search'
              ...
              html << "observeSqlField(\'issue_custom_field_values_#{field.id}\', \'#{Redmine::Utils.relative_url_root}/custom_sql_search/search?project_id=#{context[:issue].project_id}&custom_field_id=#{field.id}\', obj)\n"
              html << "//]]>\n"
              html << "</script>\n"
          end
        end
